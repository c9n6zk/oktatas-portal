...
export default async function AdminAssignmentsPage() {
  ...
  const [assignments, subjects, classes, groups, teachers] = await Promise.all([
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          {assignments.length} hozzárendelés · adott évben mely oktató tanítja mely osztálynak/csoportnak mely tárgyat.
        </p>
      </div>
      ...
      <AssignmentsPanel
        initial={assignments.map((a) => ({
          ...
          target: a.schoolClass
            ? `${a.schoolClass.startYear}/${a.schoolClass.identifier}`
            : a.group
              ? `Csoport: ${a.group.name}`
              : "—",
          targetKind: (a.schoolClass ? "class" : "group") as "class" | "group",
